...
def fedtrimmedavg_strategy(initial_parameters, client_config, strategy_name, adpater_name="default", seed=42):
    """
    Implement FedAvg Strategy
    """
    class FedTrimmedAvgModelStrategy(fl.server.strategy.FedTrimmedAvg):
        def aggregate_fit(
            self,
            server_round: int,
    ...
    strategy = FedTrimmedAvgModelStrategy(
        fraction_fit = 0.8,
